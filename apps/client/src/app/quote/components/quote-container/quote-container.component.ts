...
export class QuoteContainerComponent implements OnInit, OnDestroy {
  ...
  constructor(private quoteService: QuoteService, private modalService: NgxSmartModalService) {}
  ...
  private listenToShareModalEvents(modal: NgxSmartModalComponent): void {
    ...
    combineLatest([modal.onClose, modal.onDataAdded])
      .pipe(
    ...
    race(modal.onCloseFinished, modal.onDismissFinished)
      .pipe(
        take(1),
